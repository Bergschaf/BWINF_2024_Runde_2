...
\begin{document}
    ...
    \begin{proof}
        ...
        \end{enumerate}
        Für die Anzahl der Blätter auf Ebene $0, \dots, i + 1$ gilt jetzt:
        \begin{equation}
            \begin{aligned}
        ...
        Da wir nun $l_2 + 1$ Blätter aus Ebene $i +1$ kürzen müssen


        Erstmal für $c_1 = c_2 = 1, c_3 = 2$ \\
        $sig_i = (m\dots)$
        -> $q$ darf maximal bis $q = n - (m + l_1)$ -> dass beim reduzieren mindestens zwei dableiben\\
        Erlaubte Blätter auf der Ebene $i + 2$: $n - (m + l_1)$.
        Wenn $q$ größer des, dann Blättersituation auf Ebene $i$: $l_1 - q$\\
        Blätter auf Ebene $i + 1$: $l_2 + 2 \cdot q$ \\
        Gesamtblätter: $m + l_1 - q + l_2 + 2 \cdot q = m + l_1 + l_2 + q$ \\
        Wenn $q >n - (m + l_1)$, also $q = n - (m + l_1) + x, x > 0$ \\
        Gesamtblätter: $l_2 + n + x$ \\
        Dann werden $l_2$ Blätter wegreduziert und mindestens eins dazu.
        Des ist aber schlecht, weil dann hat einer der expandierten knoten nur 1 blatt und damit kann der baum nichtmehr optimal sein (bzw. man hätte weniger expandieren sollen)
    \end{proof}

...
\begin{document}
    ...
    \end{korollar}
    Da $Fill(T_{opt})$ höchstens $n (r -1)$ Blätter hat können wir $Fill(T_{opt})$ finden, indem wir vom Wurzel-Baum % TODo Wurzel Baum definieren
    ...
    TODO genauer so ähnlich wie im paper.
    ...
    Die \textit{For}-Schleife hat wieder eine asymptotische Laufzeit von $O(C \cdot n(r-1))$, da sie $O(n(r-1))$ mal ausgeführt wird und die \textit{Expand} und \textit{Reduce} Operationen jeweils eine Laufzeit von $O(C)$ haben. Insgesamt hat der Algorithmus also eine Asymptotische Laufzeit von $O(C \cdot n(r-1) \cdot n ^ {C + 1})$. \\

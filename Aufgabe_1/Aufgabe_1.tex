...
\begin{document}
    ...
    ....
    TODO erklären, dass damit alle gefunden werden

    \subsubsection{Laufzeit}
    TODO

    \subsection{Optimierung} % TODO besserer Titel
    ...
    \begin{definition}
        TODO
        $S \le S'$ if \[(m + l_1 + \dots + l_C, m + l_1 + \dots + l_{C-1}, \dots, m + l_1, m])\]
        Lexigraphisch kleiner (linker eintrag zählt am meisten)
        \[(m' + l_1' + \dots + l_C', m' + l_1' + \dots + l_{C-1}', \dots, m' + l_1', m'])\]
    \end{definition}
    \begin{lemma}
    ...
    \begin{proof}
        ...
        Die Verallgemeinerung dieses Beweises ist trivial und wird daher dem Leser überlassen. (Falls der Leser sich eine Lösung wünscht, ist diese im Anhang zu finden.)
    \end{proof}
    \begin{proof}
        ...
        Für die Anzahl aller Blätter auf den Ebenen $0, \dots, i + c_2 - 1$ gilt jetzt:
        \begin{equation}
            \begin{aligned}
                m' + \sum^{c_2}_{j=1} l_j' &=  m + l_1 - q + \left(\sum^{c_2-1}_{j=1} l_{j+1} + q \cdot d_j\right)\\
                %TODO den schritt erklären (weil d_j nur einmal = 1)(NICHT ZWINGED, vlt auch garkeinmal = 1) für j < c_2
                &= m + l_1 - q + \left(\sum^{c_2-1}_{j=1} l_{j+1}\right) + q\\
                &= m + l_1 + \left(\sum^{c_2-1}_{j=1} l_{j+1}\right)\\
        ...
        \end{equation}
        Daraus bilden wir die Differenz, um die Anzahl der Blätter auf der Ebene $i + c_2$ zu berechnen:

        \begin{equation}
            \begin{aligned}
                \left(m + l_1 + l_{c_2 + 1} + q \cdot d_{c_2} + \sum^{c_2-1}_{j=1} l_{j+1}\right) - \left(m + l_1 + \left(\sum^{c_2-1}_{j=1} l_{j+1}\right)\right) = l_{c_2+1} + q \cdot d_{c_2}
            \end{aligned}\label{eq:equation132}
        \end{equation}
    \end{proof}
    Um nun mit ...
    Wir müssen zeigen, dass die Anzahl aller Blätter auf den Ebenen $0, \dots, i + c_2$ größer als $n$ ist und wir reduzieren müssen:

    \begin{equation}
        \begin{align}
            m + l_1 + l_{c_2 + 1} + q \cdot d_{c_2} + \sum^{c_2-1}_{j=1} l_{j+1} &=m + l_1 + l_{c_2 + 1} + q + q \cdot (d_{c_2} - 1)  + \sum^{c_2-1}_{j=1} l_{j+1} \\
            &= m + l_1 +  \left(n - (m + \sum^{c_2}_{j=1} l_j) + x\right) + q \cdot (d_{c_2} - 1) + \sum^{c_2-1}_{j=1} l_{j+1} \\
            &= m + l_{c_2 + 1} +  \left(n - (m + \sum^{c_2}_{j=1} l_j) + x\right) + q \cdot (d_{c_2} - 1) + \sum^{c_2}_{j=1} l_{j} \\
            &= n + l_{c_2 + 1} + q \cdot (d_{c_2} - 1) + x\\
        \end{align}\label{eq:equation4}
    \end{equation}

    \subsection{Heuristik}
    Da dieser Algorithmus für große Eingabealphabete ($n >> 100$) deutlich zu langsam (Die i32 counter variable ist mehrfach overflowed) ist und zu viel Speicher verwendet, ...

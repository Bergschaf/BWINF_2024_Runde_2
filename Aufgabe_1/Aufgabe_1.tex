...
\begin{document}
    ...
    \begin{definition}
        TODO Opt kann man sich glaub schenken
    ...
    \begin{lemma}
        ...
        ...\\
        $T_1 = T_2 \to cost(T_1) = cost(T_2)$??
        $T_1 = T_2$ -> gleiche Anzahl an knoten auf jeder Ebene (weil vollständig???)
    \end{lemma}
    \begin{lemma}
        Jeder vollständige Baum $T$ kann (up to Equivalence???) mit mehreren $Expand()$ Operationen vom (root baum?) aus konstruiert werden
    \end{lemma}

    \begin{lemma}
        WICHTIG \\
        $cost(Fill(T_{opt})) = cost(T_{opt})$ -> ist glaub schon fix \\
        $Fill(T_{opt})$ wird von der naiven Implementierung gefunden () \\
    ...
    TODO (wieso darf man bfs, eigenlich doch dijkstra?) des eine ist gar kein bfs!!!!
